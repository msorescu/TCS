    ...
    protected public void getOutputXML() throws Exception {
        ...
        while (rec != null)
        {
            ...
            for (int i = 0;i < getResultField().length;i++)
            {
                ...
                {
                    case Tcs.Mls.TCSStandardResultFields.STDF_AG_AGENTID: 
                        fieldValue = rec.getTCSStdFieldValue(getResultField()[i],false);
                        recordID = fieldValue;
                        AgentID = fieldValue;
                        break;
                    case Tcs.Mls.TCSStandardResultFields.STDF_AG_OFFICEID: 
                        fieldValue = rec.getTCSStdFieldValue(getResultField()[i],false);
                        OfficeID = fieldValue;
                        break;
                    case Tcs.Mls.TCSStandardResultFields.STDF_AG_AGENTIDTIGERLEAD: 
                ...
                if (bAddField && !(StringSupport.isNullOrEmpty(fieldValue)))
                    m_resultBuffer.Append("<" + fieldName + ">" + Tcs.Mls.Util.convertStringToXML(Tcs.Mls.Util.filterJunkValue(fieldValue)) + "</" + fieldName + ">");
